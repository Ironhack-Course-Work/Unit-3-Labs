-- Lab 7 | SQL Rolling calculations
			#course work 'sql day 8 views'

    1. Get number of monthly active customers.
    
 with cte_rental_activity as 
 (
 select date_format(convert(rental_date, date),'%y-%m') as rentaldate_y_m,
 count(distinct customer_id) as customer_amount
 from rental
 group by rentaldate_y_m
 )
 Select * from cte_rental_activity

    
    2. Active users in the previous month.
    
Select *, lag(rentaldate_y_m) as last_month from cte_rental_activity
